...
class AttendanceAgent:
    ...
    def backfill(self, range_from: date | None, range_to: date | None, *, all_history: bool = False,
        ...
        """Import date-bounded AcsEvent history without touching live queue/cursor state."""
        ...
        while not self.stop_requested.is_set():
            ...
            for raw in raw_events:
                ...
                if not all_history and (event_day < range_from or event_day > range_to):
                    continue
                selected.append(event); totals["matched"] += 1
            ...
            if not dry_run and selected:
                ...
                totals["already_existing"] += duplicates; totals["delivered"] += acknowledged - duplicates
            ...
            if finished:
                return totals | {"oldest_event_time": oldest, "newest_event_time": newest, "complete": True, "dry_run": dry_run}
            position = next_position
        return totals | {"oldest_event_time": oldest, "newest_event_time": newest, "complete": False, "dry_run": dry_run}
...
def main() -> int:
    ...
    if args.command == "backfill":
        if args.all and (args.range_from or args.range_to): parser.error("backfill accepts --all or --from/--to, not both")
        if not args.all and not (args.range_from and args.range_to): parser.error("backfill requires --all or both --from YYYY-MM-DD and --to YYYY-MM-DD")
        try:
        ...
        except ValueError: parser.error("backfill dates must use YYYY-MM-DD")
        if range_from and range_from > range_to: parser.error("backfill --from must be on or before --to")
    ...
    try:
        ...
        if args.command == "test-device":
            ...
            for label, key in (("Scanned", "scanned"), ("Matched attendance", "matched"), ("Delivered", "delivered"), ("Already existing", "already_existing"), ("Unmapped", "unmapped"), ("Rejected", "rejected"), ("Failed", "failed"), ("Oldest imported event", "oldest_event_time"), ("Newest imported event", "newest_event_time")):
